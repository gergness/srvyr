# Copied from survey::svyby -- need to fix a bug
svyby_fix <-function (formula, by, design, FUN, ..., deff = FALSE, keep.var = TRUE,
                      ...
                      covmat = FALSE, return.replicates = FALSE, na.rm.by = FALSE,
                      na.rm.all = FALSE, multicore = getOption("survey.multicore"))
{
  ...
  nvartype <- which(eval(formals(sys.function())$vartype) %in%
                      vartype)
  ...
  if (keep.var) {
    ...
    if (covmat || return.replicates) {
      replicates <- do.call(cbind, lapply(results, "[[",
                                          "replicates"))
  ...
  attr(rval, "svyby") <- list(margins = 1:NCOL(byfactors),
                              ...
                              deffs = deff, statistic = deparse(substitute(FUN)), variables = names(rval)[-(1:NCOL(byfactors))][1:nstats],
                              vartype = vartype)
  if (!keep.names)
