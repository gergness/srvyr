...
data <- data.frame(id1 = 1:2, id2 = 3:4, repwt1 = 1:0, repwt2 = 0:1, wt = c(1/4, 3/4), x = 10:11)
...
test_that("as_survey correctly uses survey design's arguments",
          expect_equal(as_survey(data, ids = c(id1, id2), weights = wt,
                       ...
                       design_survey(data, ids = c(id1, id2), weights = wt,
                                     strata = repwt1, variables = x, pps = pps)))

test_that("as_survey_ correctly uses survey design's arguments",
          expect_equal(as_survey_(data, ids = c("id1", "id2"), weight = "wt",
                                 strata = "repwt1", variables = "x", pps = pps),
                       design_survey_(data, ids = c("id1", "id2"), weight = "wt",
                                     strata = "repwt1", variables = "x", pps = pps)))


# Replicates
test_that("as_survey correctly uses replicate's arguments",
          expect_equal(as_survey(data, repweights = repwt1:repwt2, weight = wt,
                                variables = x),
                       design_survey_rep(data, repweights = repwt1:repwt2, weight = wt,
                                         variables = x)))

test_that("as_survey_ correctly uses replicate's arguments",
          expect_equal(as_survey_(data, repweights = c("repwt1", "repwt2"), weight = "wt",
                                  variables = "x"),
                       design_survey_rep_(data, repweights = c("repwt1", "repwt2"), weight = "wt",
                                          variables = "x")))
...
test_that("as_survey correctly uses twophase's arguments",
          expect_equal(mu284_1 %>%
                         as_survey_(id = list("id1", "id"), strata = list(NULL, "id1"),
                                   fpc = list("n1", NULL), subset = "sub"),
                       mu284_1 %>%
                         design_twophase_(id = list("id1", "id"), strata = list(NULL, "id1"),
                                         fpc = list("n1", NULL), subset = "sub")))
