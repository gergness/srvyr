...
The meat of the `sryvr` package is the `design_survey` function. This is 
similar to the `svydesign` function from the `survey` package, but with 
some `dplyr` like functionality. `design_survey` is different from 
...
`mutate`, and `group_by`. Here are some basic examples using `design_survey` 
