...
progress indicator scorevaried by school level and percent of parents were 
...
`as_survey_design()` and `as_survey_rep()` are analagous to
`survey::svydesign()` and `survey::svrepdesign()` respectively. Because they are
designed to match `dplyr`'s style of non-standard evaluation, they accept bare
column names instead of formulas (~). They also move the data argument first, so
that it is easier to use `magrittr` pipes (`%>%`).
...
calculate summary statistics: `survey_mean()`, `survey_total()`,
